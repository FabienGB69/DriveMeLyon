      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {services.map((service) => (
            <article
              key={service.id}
              className="bg-white rounded-sm border border-black/[0.03] shadow-sm overflow-hidden group transition-all duration-300 hover:-translate-y-2 hover:shadow-lg hover:border-[#C6A15B]/25 flex flex-col"
            >
              ...
              <div className="p-8 flex flex-col flex-grow">
                <h3 className="font-serif text-2xl text-[#080808] mb-3">{service.title}</h3>
                <p className="text-gray-500 text-sm leading-relaxed flex-grow mb-6">
                  {service.description[lang]}
                </p>
                <Link
                  href={service.slug[lang]}
                  className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#C6A15B] group-hover:gap-3 transition-all"
                >
