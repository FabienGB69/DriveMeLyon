    ...
    <section className="bg-[#f3f1eb] py-24">
      <div className="max-w-3xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <h2 className="font-serif text-4xl md:text-5xl text-[#0a0a0a]">
        ...
        <div className="space-y-4">
          {items.map((faq) => (
            <div
              key={faq.id}
              className="bg-white border border-black/5 rounded-sm overflow-hidden transition-all hover:border-[#c5a880]/25 hover:shadow-sm"
            >
              <button
                className="w-full flex items-center justify-between text-left px-7 py-5 font-sans font-semibold text-[#0a0a0a]"
                onClick={() => setOpen(open === faq.id ? null : faq.id)}
                aria-expanded={open === faq.id}
              >
                <span>{faq.question[lang]}</span>
                <svg
                  className={`w-4 h-4 text-[#c5a880] flex-shrink-0 ml-4 transition-transform ${open === faq.id ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
